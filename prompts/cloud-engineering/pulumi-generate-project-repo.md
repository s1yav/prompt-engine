...
2. **Pulumi Initialization:** Run `pulumi new gcp-typescript --name {{PROJECT_NAME}} --stack dev --yes --non-interactive` to pull down the official template base.
3. **Secure Config Injection:** Run the native encryption command to hide your project ID: `pulumi config set gcp:project {{GCP_PROJECT_ID}} --secret`.
4. **Git Bootstrap:** 
...
5. **GitHub Remote Provisions:** Use the GitHub CLI (`gh`) to create the remote repository and push your branches directly:
